...
# trim
source "$TARGET/configrc"
python3 -m trimAFL -f -r "$OUT/$P" "$TMP_DIR/Ftargets.txt" 2>&1 | tee /dev/stderr | grep 'Trim-number' | awk '{print $NF}' > "$TMP_DIR/$BUG.trim"
echo "$(cat $TMP_DIR/$BUG.trim) blocks trimmed for $BUG in $P"
